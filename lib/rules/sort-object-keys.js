...
/**
 * @function getSortingPropertyName
 *
 * @description
 * get the name used for sorting purposes
 *
 * @param {Object} property the property to get the name from
 * @returns {string} the property name used for sorting
 */
  ...
  create(context) {
    ...
    return {
      ...
      Property(node) {
        ...
        var propertyNames = node.parent.properties.map(getSortingPropertyName);
        ...
        if (!shouldIgnoreOrder && sorter(prevName, thisName) > 0) {
          context.report({
            ...
            fix(fixer) {
              ...
              var propertyListIncludesComplexComputedProperties = node.parent.properties.some(function(property) {
                return property.computed && property.key && property.key.object;
              });

              if (propertyListIncludesComments || propertyListIncludesComplexComputedProperties) {
                return null;
              }
